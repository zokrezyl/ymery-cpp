// hex-editor widget plugin - Hex editor widget
// Based on imgui_hex_editor by Teselka (https://github.com/Teselka/imgui_hex_editor)
...
enum ImGuiHexEditorHighlightFlags_ : int {
  ...
  ImGuiHexEditorHighlightFlags_FullSized =
      1 << 2, // Highlight entire byte space including it's container, has no
              // effect on ascii
  ImGuiHexEditorHighlightFlags_Ascii =
      1 << 3, // Highlight ascii (doesn't affect single byte highlighting)
...
typedef int
    ImGuiHexEditorHighlightFlags; // -> enum ImGuiHexEditorHighlightFlags_ //
                                  // Flags: for ImGuiHexEditor callbacks
...
enum ImGuiHexEditorClipboardFlags_ : int {
  ...
  ImGuiHexEditorClipboardFlags_Multiline =
      1 << 0, // Separate resulting hex editor lines with carriage return
};

typedef int
    ImGuiHexEditorClipboardFlags; // -> enum ImGuiHexEditorClipboardFlags_
...
struct ImGuiHexEditorState {
  ...
  int (*ReadCallback)(ImGuiHexEditorState *state, int offset, void *buf,
                      int size) = nullptr;
  int (*WriteCallback)(ImGuiHexEditorState *state, int offset, void *buf,
                       int size) = nullptr;
  bool (*GetAddressNameCallback)(ImGuiHexEditorState *state, int offset,
                                 char *buf, int size) = nullptr;
  ImGuiHexEditorHighlightFlags (*SingleHighlightCallback)(
      ImGuiHexEditorState *state, int offset, ImColor *color,
      ImColor *text_color, ImColor *border_color) = nullptr;
  void (*HighlightRangesCallback)(ImGuiHexEditorState *state, int display_start,
                                  int display_end) = nullptr;
...
bool CalcHexEditorRowRange(int row_offset, int row_bytes_count, int range_min,
                           int range_max, int *out_min, int *out_max);
} // namespace ImGui

static char HalfByteToPrintable(unsigned char half_byte, bool lower) {
  IM_ASSERT(!(half_byte & 0xf0));
  return half_byte <= 9 ? '0' + half_byte
                        : (lower ? 'a' : 'A') + half_byte - 10;
}

static unsigned char KeyToHalfByte(ImGuiKey key) {
  IM_ASSERT((key >= ImGuiKey_A && key <= ImGuiKey_F) ||
            (key >= ImGuiKey_0 && key <= ImGuiKey_9));
  return (key >= ImGuiKey_A && key <= ImGuiKey_F)
             ? (char)(key - ImGuiKey_A) + 10
             : (char)(key - ImGuiKey_0);
}
...
static int CalcBytesPerLine(float bytes_avail_x, const ImVec2 &byte_size,
                            const ImVec2 &spacing, bool show_ascii,
                            const ImVec2 &char_size, int separators) {
  const float byte_width =
      byte_size.x + spacing.x + (show_ascii ? char_size.x : 0.f);
  int bytes_per_line = (int)(bytes_avail_x / byte_width);
  bytes_per_line = bytes_per_line <= 0 ? 1 : bytes_per_line;

  int actual_separators =
      separators > 0 ? (int)(bytes_per_line / separators) : 0;
  if (actual_separators != 0 && separators > 0 &&
      bytes_per_line > actual_separators &&
      (bytes_per_line - 1) % actual_separators == 0)
    --actual_separators;

  return separators > 0
             ? CalcBytesPerLine(bytes_avail_x - (actual_separators * spacing.x),
                                byte_size, spacing, show_ascii, char_size, 0)
             : bytes_per_line;
}
...
static ImColor CalcContrastColor(ImColor color) {
  ...
  const float l =
      (0.299f * (color.Value.z * 255.f) + 0.587f * (color.Value.y * 255.f) +
       0.114f * (color.Value.x * 255.f)) /
      255.f;
#else
  const float l =
      (0.299f * (color.Value.x * 255.f) + 0.587f * (color.Value.y * 255.f) +
       0.114f * (color.Value.z * 255.f)) /
      255.f;
#endif
...
static bool RangeRangeIntersection(int a_min, int a_max, int b_min, int b_max,
...
static void RenderRectCornerCalcRounding(const ImVec2 &ra, const ImVec2 &rb,
                                         float &rounding) {
  rounding = ImMin(rounding, ImFabs(rb.x - ra.x) * 0.5f);
  rounding = ImMin(rounding, ImFabs(rb.y - ra.y) * 0.5f);
}

static void RenderTopLeftCornerRect(ImDrawList *draw_list, const ImVec2 &a,
                                    const ImVec2 &b, ImColor color,
                                    float rounding) {
...
static void RenderBottomRightCornerRect(ImDrawList *draw_list, const ImVec2 &a,
                                        const ImVec2 &b, ImColor color,
                                        float rounding) {
...
static void RenderTopRightCornerRect(ImDrawList *draw_list, const ImVec2 &a,
                                     const ImVec2 &b, ImColor color,
                                     float rounding) {
  const ImVec2 ra = {a.x + 0.5f, a.y + 0.5f};
...
static void RenderBottomLeftCornerRect(ImDrawList *draw_list, const ImVec2 &a,
                                       const ImVec2 &b, ImColor color,
                                       float rounding) {
  const ImVec2 ra = {a.x + 0.5f, a.y + 0.5f};
  const ImVec2 rb = {b.x + 0.5f, b.y + 0.5f};
...
static void RenderBottomCornerRect(ImDrawList *draw_list, const ImVec2 &a,
                                   const ImVec2 &b, ImColor color,
                                   float rounding) {
...
static void RenderTopCornerRect(ImDrawList *draw_list, const ImVec2 &a,
                                const ImVec2 &b, ImColor color,
                                float rounding) {
  const ImVec2 ra = {a.x + 0.5f, a.y + 0.5f};
  const ImVec2 rb = {b.x - 0.5f, b.y + 0.5f};
...
static void RenderByteDecorations(ImDrawList *draw_list, const ImRect &bb,
                                  ImColor bg_color,
                                  ImGuiHexEditorHighlightFlags flags,
                                  ImColor border_color, float rounding,
                                  int offset, int range_min, int range_max,
                                  int bytes_per_line, int i, int line_base) {
  ...
  if (offset == range_min) {
    if (!is_last_byte) {
      ...
      RenderTopLeftCornerRect(draw_list, bb.Min, bb.Max, border_color,
                              rounding);

      if (start_line == end_line)
        draw_list->AddLine({bb.Min.x, bb.Max.y}, {bb.Max.x, bb.Max.y},
                           border_color);
    } else {
      draw_list->AddRectFilled(bb.Min, bb.Max, bg_color, rounding,
    ...
    if (is_end_line) {
      if (offset == range_max) {
        ...
        RenderBottomCornerRect(draw_list, bb.Min, bb.Max, border_color,
                               rounding);
      } else {
        draw_list->AddRectFilled(bb.Min, bb.Max, bg_color, rounding,
                                 ImDrawFlags_RoundCornersBottomLeft);
        RenderBottomLeftCornerRect(draw_list, bb.Min, bb.Max, border_color,
                                   rounding);
      }

      rendered_bg = true;
    } else if (current_line == start_line + 1 &&
               (range_min % bytes_per_line) != 0) {
      draw_list->AddRectFilled(bb.Min, bb.Max, bg_color, rounding,
                               ImDrawFlags_RoundCornersTopLeft);
      RenderTopLeftCornerRect(draw_list, bb.Min, bb.Max, border_color,
                              rounding);
      ...
      draw_list->AddLine({bb.Min.x, bb.Min.y}, {bb.Min.x, bb.Max.y},
                         border_color);
    }
  }
  ...
  if (i != 0 && offset == range_max) {
    if (start_line == end_line) {
      ...
      RenderTopRightCornerRect(draw_list, bb.Min, bb.Max, border_color,
                               rounding);
      draw_list->AddLine({bb.Min.x, bb.Max.y}, {bb.Max.x, bb.Max.y},
                         border_color);
    } else {
      ...
      RenderBottomRightCornerRect(draw_list, bb.Min, bb.Max, border_color,
                                  rounding);
    }
    ...
    if (is_start_line) {
      ...
      rendered_bg = true;
               ...
               (range_max % bytes_per_line) != bytes_per_line - 1) {
      draw_list->AddRectFilled(bb.Min, bb.Max, bg_color, rounding,
                               ImDrawFlags_RoundCornersBottomRight);
      RenderBottomRightCornerRect(draw_list, bb.Min, bb.Max, border_color,
                                  rounding);
      ...
      draw_list->AddLine({bb.Max.x - 1.f, bb.Min.y}, {bb.Max.x - 1.f, bb.Max.y},
                         border_color);
    }
  }

  if ((is_start_line && offset != range_min && !is_last_byte &&
       offset != range_max) ||
      (current_line == start_line + 1 &&
       (i < (range_min % bytes_per_line) && i != 0))) {
    if (!rendered_bg) {
      draw_list->AddRectFilled(bb.Min, bb.Max, bg_color, 0.f);
      rendered_bg = true;
    }

    draw_list->AddLine({bb.Min.x, bb.Min.y}, {bb.Max.x, bb.Min.y},
                       border_color);
  }

  if ((is_end_line && offset != range_max && i != 0) ||
      (current_line == end_line - 1 &&
       (i > (range_max % bytes_per_line) && !is_last_byte))) {
    if (!rendered_bg) {
      draw_list->AddRectFilled(bb.Min, bb.Max, bg_color, 0.f);
      rendered_bg = true;
    }
...
bool ImGui::BeginHexEditor(const char *str_id, ImGuiHexEditorState *state,
                           const ImVec2 &size, ImGuiChildFlags child_flags,
  ...
  if (state->ShowAddress) {
    ...
    if (address_chars == -1)
      address_chars =
          ImFormatString(nullptr, 0, "%zX", (size_t)state->MaxBytes) + 1;
  ...
  if (state->BytesPerLine == -1) {
    bytes_per_line = CalcBytesPerLine(bytes_avail_x, byte_size, spacing,
                                      show_ascii, char_size, state->Separators);
  } else {
  ...
  const ImColor text_selected_bg_color =
  ...
  if (state->EnableClipboard &&
    ...
    if (state->SelectStartByte != -1) {
      const int bytes_count =
          (state->SelectEndByte + 1) - state->SelectStartByte;
      ...
      if (bytes) {
        ...
        if (state->ReadCallback)
          read_bytes = state->ReadCallback(state, state->SelectStartByte, bytes,
                                           bytes_count);
        else {
          memcpy(bytes, (char *)state->Bytes + state->SelectStartByte,
                 bytes_count);
        ...
        if (read_bytes > 0) {
          ...
          for (int i = 0, abs_i = state->SelectStartByte; i < bytes_count;
               i++, abs_i++) {
            ...
            if (bytes_per_line != 0 &&
                ((abs_i % bytes_per_line) == bytes_per_line - 1) &&
                abs_i != 0) {
    ...
    for (ImGuiKey key = ImGuiKey_A; key != ImGuiKey_G;
    ...
    if (hex_key_pressed == ImGuiKey_None) {
      for (ImGuiKey key = ImGuiKey_0; key != ImGuiKey_A;
           key = (ImGuiKey)((int)key + 1)) {
        if (ImGui::IsKeyPressed(key)) {
          hex_key_pressed = key;
          break;
  ...
  unsigned char *line_buf =
      bytes_per_line <= sizeof(stack_line_buf)
          ? stack_line_buf
          : (unsigned char *)ImGui::MemAlloc(bytes_per_line);
  if (!line_buf)
    return true;

  char stack_address_buf[32];
  char *address_buf = address_max_chars <= sizeof(stack_address_buf)
                          ? stack_address_buf
                          : (char *)ImGui::MemAlloc(address_max_chars);
  if (!address_buf) {
  ...
  while (clipper.Step()) {
    ...
    ImVec2 ascii_cursor = {cursor.x + address_max_size + (spacing.x * 0.5f) +
                               (bytes_per_line * (byte_size.x + spacing.x)) +
    ...
    if (show_ascii) {
      draw_list->AddLine(
          ascii_cursor,
          {ascii_cursor.x,
           ascii_cursor.y + clipper_lines * (byte_size.y + spacing.y)},
          separator_color);
    }
    ...
    for (int n = clipper.DisplayStart; n != clipper.DisplayEnd; n++) {
      ...
      if (state->ShowAddress) {
        if (!state->GetAddressNameCallback ||
            !state->GetAddressNameCallback(state, line_base, address_buf,
                                           address_max_chars))
          ImFormatString(address_buf, (size_t)address_max_chars, "%0.*zX",
                         address_max_chars - 1, (size_t)line_base);

        const ImVec2 text_size = ImGui::CalcTextSize(address_buf);
        draw_list->AddText(cursor, text_color, address_buf);
        draw_list->AddText({cursor.x + text_size.x, cursor.y},
                           text_disabled_color, ":");
        cursor.x += address_max_size;
      }

      int max_bytes_per_line = line_base;
      max_bytes_per_line = max_bytes_per_line > state->MaxBytes
                               ? max_bytes_per_line - state->MaxBytes
                               : bytes_per_line;
      ...
      if (!state->ReadCallback) {
        ...
        bytes_read =
            state->ReadCallback(state, line_base, line_buf, max_bytes_per_line);

      cursor.x += spacing.x * 0.5f;

      for (int i = 0; i != bytes_per_line; i++) {
        const ImRect byte_bb = {
            {cursor.x, cursor.y},
            {cursor.x + byte_size.x, cursor.y + byte_size.y}};
        ...
        ImColor byte_text_color =
            (offset >= state->MaxBytes ||
             (state->RenderZeroesDisabled && byte == 0x00) || i >= bytes_read)
                ? text_disabled_color
                : text_color;
        ...
        if (offset >= select_start_byte && offset <= select_end_byte) {
          ...
          ImRect bb = (flags & ImGuiHexEditorHighlightFlags_FullSized)
                          ? item_bb
                          : byte_bb;
          ...
          RenderByteDecorations(draw_list, bb, text_selected_bg_color, flags,
                                border_color, style.FrameRounding, offset,
                                select_start_byte, select_end_byte,
          ...
          if (flags & ImGuiHexEditorHighlightFlags_Ascii) {
            RenderByteDecorations(
                draw_list,
                {byte_ascii,
                 {byte_ascii.x + char_size.x, byte_ascii.y + char_size.y}},
                text_selected_bg_color, flags, border_color,
                style.FrameRounding, offset, offset, offset, bytes_per_line, i,
                line_base);
          }
        } else {
          ...
          if (state->SingleHighlightCallback) {
            ...
            ImGuiHexEditorHighlightFlags flags = state->SingleHighlightCallback(
                state, offset, &color, &byte_text_color, &custom_border_color);
            ...
            if (flags & ImGuiHexEditorHighlightFlags_Apply) {
              ...
              RenderByteDecorations(
                  draw_list,
                  (flags & ImGuiHexEditorHighlightFlags_FullSized) ? item_bb
                                                                   : byte_bb,
                  color, flags, highlight_border_color, style.FrameRounding,
                  offset, offset, offset, bytes_per_line, i, line_base);
          ...
          if (!single_highlight) {
            for (int j = 0; j != state->HighlightRanges.Size; j++) {
              ...
              if (line_base + i >= range.From && line_base + i <= range.To) {
                ...
                RenderByteDecorations(
                    draw_list,
                    (range.Flags & ImGuiHexEditorHighlightFlags_FullSized)
                        ? item_bb
                        : byte_bb,
                    range.Color, range.Flags, highlight_border_color,
                    style.FrameRounding, offset, range.From, range.To,
                    bytes_per_line, i, line_base);

                if (range.Flags & ImGuiHexEditorHighlightFlags_Ascii) {
                  RenderByteDecorations(draw_list,
                                        {byte_ascii,
                                         {byte_ascii.x + char_size.x,
                                          byte_ascii.y + char_size.y}},
                                        range.Color, range.Flags,
                                        highlight_border_color,
                                        style.FrameRounding, offset, range.From,
                                        range.To, bytes_per_line, i, line_base);
                }
        ...
        if (offset == select_start_byte) {
          ...
          if (!io.ConfigInputTextCursorBlink ||
              ImFmod(state->SelectCursorAnimationTime, 1.20f) <= 0.80f) {
            ImVec2 pos;
            pos.x = byte_bb.Min.x;
            pos.y = byte_bb.Max.y;
            ...
            draw_list->AddLine({pos.x, pos.y}, {pos.x + char_size.x, pos.y},
                               text_color);
          }
        }

        const bool hovered =
            ImGui::ItemHoverable(item_bb, id, ImGuiItemFlags_Inputable);

        if (select_drag_byte != -1 && offset == select_drag_byte &&
            !mouse_left_down) {
        ...
        if (offset == next_last_selected_byte &&
            last_selected_byte != next_last_selected_byte) {
          ImGui::SetKeyboardFocusHere();
        }

        if (offset == last_selected_byte && !state->ReadOnly &&
            hex_key_pressed != ImGuiKey_None) {
          IM_ASSERT(offset == select_start_byte || offset == select_end_byte);
          const int subbyte = offset == select_start_byte ? select_start_subbyte
                                                          : select_end_subbyte;
          ...
          if (!state->WriteCallback)
            *(unsigned char *)((char *)state->Bytes + n * bytes_per_line + i) =
                wbyte;
          else
            state->WriteCallback(state, n * bytes_per_line + i, &wbyte,
                                 sizeof(wbyte));

          int *next_subbyte =
              (int *)(offset == select_start_byte ? &next_select_start_subbyte
                                                  : &next_select_end_subbyte);
          if (!subbyte) {
            next_select_start_byte = offset;
            next_select_end_byte = offset;
            *next_subbyte = 1;
        ...
        if (i > 0 && state->Separators > 0 &&
            (i + 1) % state->Separators == 0 && i != bytes_per_line - 1)
          cursor.x += spacing.x;
        ...
        if (show_ascii) {
          ...
          const ImRect char_bb = {
              byte_ascii,
              {byte_ascii.x + char_size.x, byte_ascii.y + char_size.y}};

          /*if (offset >= select_start_byte && offset <= select_end_byte)
          {
                  draw_list->AddRectFilled(char_bb.Min, char_bb.Max,
...
void ImGui::EndHexEditor() { ImGui::EndChild(); }

bool ImGui::CalcHexEditorRowRange(int row_offset, int row_bytes_count,
                                  int range_min, int range_max, int *out_min,
                                  int *out_max) {
  int abs_min;
  int abs_max;

  if (RangeRangeIntersection(row_offset, row_offset + row_bytes_count,
                             range_min, range_max, &abs_min, &abs_max)) {
...
public:
    static Result<WidgetPtr> create(
        ...
        auto widget = std::make_shared<HexEditor>();
        ...
        if (auto res = widget->init(); !res) {
            return Err<WidgetPtr>("HexEditor::create failed", res);
        }
        return widget;
    }
    ...
    Result<void> init() override {
        ...
        _buffer.resize(256, 0);

        // Initialize hex editor state
        _state.Bytes = _buffer.data();
        _state.MaxBytes = static_cast<int>(_buffer.size());
...
protected:
    Result<void> _pre_render_head() override {
        ...
        // Get buffer size
        if (auto res = _data_bag->get_static("buffer_size"); res) {
            if (auto bs = get_as<double>(*res)) {
                int new_size = static_cast<int>(*bs);
                if (new_size > 0 && new_size != static_cast<int>(_buffer.size())) {
                    _buffer.resize(new_size, 0);
                    _state.Bytes = _buffer.data();
                    _state.MaxBytes = new_size;
                }
            }
        }

        // Get read_only option
        if (auto res = _data_bag->get_static("read_only"); res) {
            if (auto ro = get_as<bool>(*res)) {
                _state.ReadOnly = *ro;
            }
        }

        // Get bytes_per_line option
        if (auto res = _data_bag->get_static("bytes_per_line"); res) {
            if (auto bpl = get_as<double>(*res)) {
                _state.BytesPerLine = static_cast<int>(*bpl);
            }
        }

        // Get show_ascii option
        if (auto res = _data_bag->get_static("show_ascii"); res) {
            if (auto sa = get_as<bool>(*res)) {
                _state.ShowAscii = *sa;
            }
        }

        // Get show_address option
        if (auto res = _data_bag->get_static("show_address"); res) {
            if (auto sa = get_as<bool>(*res)) {
                _state.ShowAddress = *sa;
            }
        }

        // Get lowercase option
        if (auto res = _data_bag->get_static("lowercase"); res) {
            if (auto lc = get_as<bool>(*res)) {
                _state.LowercaseBytes = *lc;
            }
        }

        // Render hex editor
        if (ImGui::BeginHexEditor(label.c_str(), &_state, size, ImGuiChildFlags_Border)) {
            ImGui::EndHexEditor();
        }

        // Update selection info in data bag
        if (_state.SelectStartByte >= 0) {
            _data_bag->set("selection_start", Value(static_cast<double>(_state.SelectStartByte)));
            _data_bag->set("selection_end", Value(static_cast<double>(_state.SelectEndByte)));
        }
...
private:
    ...
    ImGuiHexEditorState _state{};
};

} // namespace ymery::plugins

extern "C" const char* name() { return "hex-editor"; }
...
extern "C" ymery::Result<ymery::WidgetPtr> create(
    ...
    return ymery::plugins::HexEditor::create(wf, d, ns, db);
}
