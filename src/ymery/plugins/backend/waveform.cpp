// waveform backend plugin - generates standard waveforms (sine, square, triangle)
#include "../../types.hpp"
#include "../../result.hpp"
...
/**
 * WaveformDevice - generates waveform data in a background thread
 */
...
public:
    static Result<std::shared_ptr<WaveformDevice>> create(
        ...
        size_t buffer_size = 48000  // 1 second of audio
    ) {
    ...
    void _generate_waveform() {
        ...
        for (size_t i = 0; i < _period_size; ++i) {
            ...
            if (_waveform_type == "sine") {
                ...
                // Triangle wave: normalize phase to [0, 1]
...
/**
 * WaveformManager - manages waveform generators, implements TreeLike
 */
...
public:
    ...
    Result<std::vector<std::string>> get_children_names(const DataPath& path) override {
        ...
        // /available: return waveform types
        if (path_str == "/available") {
            return Ok(WAVEFORM_TYPES);
        }

        // /available/sine, /available/square, /available/triangle: return channel "0"
        for (const auto& type : WAVEFORM_TYPES) {
        ...
        // /opened: return opened device types
        if (path_str == "/opened") {
        ...
        // /opened/<type>: return channel "0" if device exists
        for (const auto& type : WAVEFORM_TYPES) {
    ...
    Result<Dict> get_metadata(const DataPath& path) override {
        ...
        // /opened/<type>/0 - opened channel with buffer
        for (const auto& type : WAVEFORM_TYPES) {
            if (path_str == "/opened/" + type + "/0") {
                ...
                if (it != _devices.end()) {
                    ...
                    return Ok(Dict{
                        ...
                        {"type", Value("audio-channel")},
                        {"category", Value("audio-channel")},
                        {"buffer", Value(device->get_buffer())}  // Expose buffer for implot-layer
                    });
                }
            }
        }
    ...
    Result<Value> get(const DataPath& path) override {
        // Get metadata and extract the last component as key
