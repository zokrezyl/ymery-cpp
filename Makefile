...
opengl: ## Build with OpenGL backend (Release)
	PATH="$(SYSTEM_PATH)" $(CMAKE) -B $(BUILD_DIR_OPENGL) $(CMAKE_GENERATOR) $(CMAKE_COMMON) $(CMAKE_RELEASE) -DYMERY_USE_WEBGPU=OFF -DYMERY_BUILD_GUI_TESTS=ON
	PATH="$(SYSTEM_PATH)" $(CMAKE) --build $(BUILD_DIR_OPENGL) --parallel
	cd $(BUILD_DIR_OPENGL) && ctest --output-on-failure

.PHONY: webgpu
webgpu: ## Build with WebGPU backend (Release)
	PATH="$(SYSTEM_PATH)" $(CMAKE) -B $(BUILD_DIR_WEBGPU) $(CMAKE_GENERATOR) $(CMAKE_COMMON) $(CMAKE_RELEASE) -DYMERY_USE_WEBGPU=ON -DYMERY_BUILD_GUI_TESTS=OFF
	PATH="$(SYSTEM_PATH)" $(CMAKE) --build $(BUILD_DIR_WEBGPU) --parallel
	cd $(BUILD_DIR_WEBGPU) && ctest --output-on-failure
